...
### (a) Scatter and expected error distribution
...
### (b) OLS assumption and expected effect
...
The Student's $t$ AICc is lower by 26.817, so it is strongly preferred.

## Reconcile

### (c) Slope comparison

The slopes are very close: 2.9908 versus 3.0192. This agrees with my prediction
that non-Normal errors would not change the slope very much.

### (d) What the rejected model gets wrong

The Normal model mainly gets the error shape wrong. It cannot fit both the
sharp center and the extreme residuals.

### (e) Error quantiles and capital buffer
...
### (a) Expected agreements and disagreements

![Pair plots for all four variables.](figures/problem3_pairplot.png){width=70%}
...
### (b) Correlation matrices and largest gap
...
$|0.7991-0.9719|=\mathbf{0.1728}$. This matches my prediction.

### (c) Why the coefficients differ

Spearman is more honest if the question is whether the variables move in the
same direction, because it measures monotonic rank dependence. Pearson answers
how strong the linear relationship is. The curved shape makes Pearson lower
...
### (a) Conditional variance and uncertainty factor
...
### (b) Does the factor depend on the observed value?

Under the multivariate Normal, this factor does not depend on the observed
value of $x_1$, because $x_1$ does not appear in the conditional variance
formula.

## Fit and reconcile

### (c) Conditional mean and 95% band
...
### (e) Coverage by distance from the mean
...
### (f) What the changing coverage means
...
### (a) Cutoff, decay, and predicted order

The series looks stationary around a constant mean. The ACF decays with an
oscillating pattern. The PACF has large spikes at lags 1 and 2 and is mostly
inside the band after lag 2. Based on this pattern, I predict an **AR(2)**
process. The isolated PACF spike at lag 9 is likely sampling noise rather than
a new cutoff.

### (b) Rule and significance band

The 95% significance band is
...
### (e) AR(2) versus AR(3), and why not use $R^2$
